...
public class ViewHolder {
    ...
    protected static class TimelineMediaViewHolder extends TimelineViewHolder {
        ...
        void bind(@NonNull Media mediaItem, boolean isSelected) {
            ...
            RequestOptions options = new RequestOptions().signature(mediaItem.getSignature()).format(DecodeFormat.PREFER_RGB_565).centerCrop().placeholder(placeholderImage).diskCacheStrategy(DiskCacheStrategy.ALL);
            Glide.with(imageView.getContext()).load(mediaItem.getUri()).apply(options).thumbnail(0.5f).into(imageView);
