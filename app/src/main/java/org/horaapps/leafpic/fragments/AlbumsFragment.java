...
public class AlbumsFragment extends BaseMediaGridFragment {
    ...
    private void displayAlbums() {
        ...
        CPHelper.getAlbums(getContext(), hidden, excuded, sortingMode(), sortingOrder()).subscribeOn(Schedulers.io()).map(album -> album.withSettings(HandlingAlbums.getSettings(db, album.getPath()))).observeOn(AndroidSchedulers.mainThread()).subscribe(album -> adapter.add(album), throwable -> {
    ...
    public View.OnClickListener getToolbarButtonListener(boolean editMode) {
        if (editMode)
            ...
            return v -> adapter.clearSelected();
    }
    ...
    private void showDeleteBottomSheet() {
        ...
        for (Album media : selected) sources.add(MediaHelper.deleteAlbum(getContext().getApplicationContext(), media));
        ProgressBottomSheet<Album> bottomSheet = new ProgressBottomSheet.Builder<Album>(R.string.delete_bottom_sheet_title).autoDismiss(false).sources(sources).listener(new ProgressBottomSheet.Listener<Album>() {
    ...
    public void onItemSelected(int position) {
        if (listener != null)
            listener.onAlbumClick(adapter.get(position));
    }
